import { users as staticUsers, type User } from "@shared/schema";
...
export class MemStorage implements IStorage {
  async validateUser(username: string, password: string): Promise<User | null> {
    // Check user1
    if (username === staticUsers.user1.username && password === staticUsers.user1.password) {
      return {
    ...
    // Check user2
    if (username === staticUsers.user2.username && password === staticUsers.user2.password) {
      return {
